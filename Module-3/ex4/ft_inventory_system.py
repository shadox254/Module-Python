def print_inventory(player: str, inventory: dict) -> None:
    ...
    for item in inventory:
        ...
        print(f"{item} ({item_stats['type']}, {item_stats['rarity']}): "
              f"{item_stats['quantity']}x @ {item_stats['price']} "
              f"gold each = {item_price} gold")
...
def transaction(giver: str, receiver: str, players_inv: dict, item: str,
                quantity_to_give: int) -> None:
    """
    ...
    if quantity_to_give > 1:
        print(f"=== Transaction: {giver.capitalize()} gives "
              f"{receiver.capitalize()} {quantity_to_give} {item}s ===")
    elif quantity_to_give == 1:
        print(f"=== Transaction: {giver.capitalize()} gives "
              f"{receiver.capitalize()} a {item} ===")
    else:
        print(f"=== Transaction error! Can't give {quantity_to_give} "
              f"{item} ===")
        print()
        return None

    if (item in players_inv[giver]
            and players_inv[giver][item]['quantity'] >= quantity_to_give):
        print("Transaction successful!")
    elif item not in players_inv[giver]:
        print(f"Transaction canceled! {giver.capitalize()} does not own "
              f"{item}")
        print()
        return None
    else:
        print(f"Transaction failed! {giver.capitalize()} does not have enough "
              f"{item}.")
        print()
        return None
    print()
    ...
    if quantity_to_give > 1:
        print(f"{giver.capitalize()} {item}s: "
              f"{players_inv[giver][item]['quantity']}")
        print(f"{receiver.capitalize()} {item}s: "
              f"{players_inv[receiver][item]['quantity']}")
    else:
        print(f"{giver.capitalize()} {item}: "
              f"{players_inv[giver][item]['quantity']}")
        print(f"{receiver.capitalize()} {item}: "
              f"{players_inv[receiver][item]['quantity']}")
    print()
...
if __name__ == "__main__":
    players_inv = {
        "alice": {
            "sword": {"type": "weapon", "rarity": "rare", "price": 500,
                      "quantity": 1},
            "potion": {"type": "consumable", "rarity": "common", "price": 50,
                       "quantity": 5},
            "shield": {"type": "armor", "rarity": "uncommon", "price": 200,
                       "quantity": 1}
        },
        "bob": {
            "Wizard's staff": {"type": "weapon", "rarity": "uncommon",
                               "price": 150, "quantity": 1},
            "magic_ring": {"type": "weapon", "rarity": "rare", "price": 300,
                           "quantity": 1},
            "cloak": {"type": "armor", "rarity": "common", "price": 100,
                      "quantity": 1},
        }
    }
